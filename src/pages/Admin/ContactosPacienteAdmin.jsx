...
const ContactosPacienteAdmin = () => {
    ...
    const buscarContactos = async (e) => {
        ...
        try {
            ...
            const resPaciente = await axios.get(`http://localhost:5000/api/admin/paciente/${idPaciente}`);
            ...
            // Obtener contactos
            const res = await axios.get("http://localhost:5000/api/admin/contactos-paciente", {
                params: { id_paciente: idPaciente }
            });
            setContactos(res.data);
            if (res.data.length === 0) {
                Swal.fire("Sin contactos", "Este paciente no tiene contactos registrados.", "info");
            }
        } catch {
            setContactos([]);
            setDatosPaciente(null);
    ...
    const handleAdd = async (e) => {
        e.preventDefault();
        try {
            await axios.post("http://localhost:5000/api/admin/contactos-paciente", {
                ...form,
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`http://localhost:5000/api/admin/contactos-paciente/${editId}`, form);
    ...
    const handleDelete = async (id_contacto) => {
        ...
        if (confirm.isConfirmed) {
            try {
                await axios.delete(`http://localhost:5000/api/admin/contactos-paciente/${id_contacto}`);
    ...
    return (
        <div className="container py-4">
            ...
            <div className="alert alert-warning" style={{ fontSize: "1rem" }}>
                <b>Nota:</b> Aquí puede <b>agregar, editar o eliminar</b> los contactos de emergencia de cualquier paciente.
            </div>
            {datosPaciente && (
                <div className="alert alert-info mb-3">
                    <b>ID:</b> {datosPaciente.id_paciente} <br />
                    <b>Nombre:</b> {datosPaciente.nombres} {datosPaciente.apellidos} <br />
                    <b>Correo:</b> {datosPaciente.correo} <br />
                    <b>Teléfono:</b> {datosPaciente.telefono || <span className="text-muted">No registrado</span>} <br />
                    <b>Dirección:</b> {datosPaciente.direccion || <span className="text-muted">No registrada</span>}
                </div>
            )}
            {/* Formulario agregar/editar */}
            <form className="row g-2 mb-4" onSubmit={editId ? handleUpdate : handleAdd}>
                <div className="col-md-2">
                    <input
                        type="text"
                        name="nombre"
                ...
                <table className="table table-hover align-middle">
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr>
                                <td colSpan={7} className="text-center">No hay contactos registrados.</td>
                            </tr>
                        ) : (
                            contactos.map((c) => (
